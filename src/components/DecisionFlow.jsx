...
import { correlacionAvanzada } from "../lib/estadistica/correlacion.js";
...
export default function DecisionFlow() {
  const columns = useWorkbookStore((s) => s.columns); const registrarResultado = useWorkbookStore((s) => s.registrarResultado);
  const [lsl, setLsl] = useState(""); const [usl, setUsl] = useState(""); const [target, setTarget] = useState(""); const [abierto, setAbierto] = useState(true); const [ejecutados, setEjecutados] = useState({});
  const numericas = useMemo(() => columns.filter((c) => c.type === "numeric" && c.values.some((v) => Number.isFinite(v))), [columns]);
  const calidad = useMemo(() => evaluarCalidadDatos(columns), [columns]); const principal = numericas[0];
  ...
  function ejecutar(id) {
    ...
    if (id === "correlacion") resultado = numericas.length < 2 ? { error: "Hacen falta al menos dos variables numéricas." } : { correlacion: correlacionAvanzada(numericas[0].values, numericas[1].values), regresion: regresionLinealSimple(numericas[0].values, numericas[1].values) };
  ...
  return <section style={{ margin: "10px 12px 0", border: "1px solid var(--border, #d7dce2)", borderRadius: 12, background: "var(--panel, #fff)", overflow: "hidden" }}>
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "10px 14px", borderBottom: abierto ? "1px solid var(--border, #d7dce2)" : "none" }}><div><strong>Flujo de decisión estadística</strong><div className="decision-flow-note">Datos → calidad → descriptivos → distribución → estabilidad → capacidad → lotes → conclusión</div></div><button type="button" className="stat-toggle" onClick={() => setAbierto((v) => !v)}>{abierto ? "Ocultar" : "Mostrar"}</button></header>
    {abierto && <>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "end", padding: "0 12px 10px" }}><label style={{ fontSize: 12 }}>LSL<input className="decision-flow-input" value={lsl} onChange={(e) => setLsl(e.target.value)} type="number" /></label><label style={{ fontSize: 12 }}>USL<input className="decision-flow-input" value={usl} onChange={(e) => setUsl(e.target.value)} type="number" /></label><label style={{ fontSize: 12 }}>Target<input className="decision-flow-input" value={target} onChange={(e) => setTarget(e.target.value)} type="number" /></label><div className="decision-flow-note">Variable principal: <b>{principal?.name || "—"}</b> · n={principal ? principal.values.filter(Number.isFinite).length : 0}</div></div>
      <div className="decision-flow-grid"><div className="decision-flow-summary"><b>Calidad:</b> {calidad.estado} · variables {calidad.variables.length}</div><div className="decision-flow-summary"><b>Conclusión provisional:</b> {conclusion}. No equivale a “validado”.</div>
        ...
        {ejecutados.correlacion && !ejecutados.correlacion.error && <div className="decision-flow-summary"><b>Asociación:</b> Pearson r={numero(ejecutados.correlacion.correlacion?.pearson?.r)} p={numero(ejecutados.correlacion.correlacion?.pearson?.p)} · Spearman ρ={numero(ejecutados.correlacion.correlacion?.spearman?.rho)} p={numero(ejecutados.correlacion.correlacion?.spearman?.p)} · Regresión R²={numero(ejecutados.correlacion.regresion?.r2)}</div>}
      </div>
    </>}
  </section>;
}
