...
export function welchAnova(columnas) {
  ...
  if (!Number.isFinite(W) || W === 0 || w.some((x) => !Number.isFinite(x))) return { error: "No se puede estimar la varianza dentro de los grupos." };
  ...
  const c = w.reduce((s, wi, i) => s + (1 - wi / W) ** 2 / (n[i] - 1), 0);
...
export function compararLotes(columnas) {
  ...
  return { grupos: grupos.map((g) => ({ nombre: g.name, n: g.values.filter(Number.isFinite).length })), anova, welch, kruskal, levene, posthoc, criterio: 0.05, posthocMetodo: "Comparaciones por pares de Welch con ajuste de Holm; no se presenta como Tukey." };
}
